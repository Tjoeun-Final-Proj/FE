...
class OwnerWalletServcie extends GetxService {
  ...
  Future<Map<String, dynamic>?> getSettlementSummary1() async {
  try {
    ...
    print("🌐 [Wallet API] 정산 요약 정보 요청 시작");
     ...
     final response = await _dio.get(
      'shipments/settlements/driver/summary',
...
Future<List<CommonWalletMonthModel>?> getSettlementList1(int year, int month) async {
  try {
    final currentToken = _tokenService.accessToken;
    ...
    final response = await _dio.get(
      'shipments/settlements/driver', // 엔드포인트
      queryParameters: {
        'year': year,
        'month': month,
      },
