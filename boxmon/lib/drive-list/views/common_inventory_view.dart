...
class CommonInvetoryView extends StatelessWidget {
      ...
      body: RefreshIndicator(
        ...
        child: Obx(() {
          ...
          if (shipmentController.isLoading.value &&
              shipmentController.inventoryList.isEmpty) {
            return Center(child: CircularProgressIndicator());
          }
          return ListView.builder(
            itemCount: shipmentController.inventoryList.length,
            itemBuilder: (context, index) =>
                _buildOrderItem(shipmentController.inventoryList[index]),
          );
        }),
      ),
    );
  ...
  Widget _buildOrderItem(InventoryModel item) {
    ...
    // 금액 포맷팅 (9,000)
    String formatPrice(int? price) => NumberFormat('#,###').format(price ?? 0);

    return GestureDetector(
      onTap: () => Get.toNamed(
        AppRoutes.shipmentDetail,
        arguments: {'shipmentId': item.shipmentId},
      ),
      child: Container(
                ...
                Column(
                  children: [
                    ...
                    Container(
                      width: 1,
                      height:
                          (item.waypoint1Address?.isNotEmpty == true &&
                              item.waypoint2Address?.isNotEmpty == true)
                          ? 80
                          : (item.waypoint1Address?.isNotEmpty == true)
                          ? 45
                          : 20,
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      // --- 🔥 경유지 1 (있을 때만 표시) ---
                      if (item.waypoint1Address != null &&
                          item.waypoint1Address!.isNotEmpty) ...[
                      ...
                      // --- 🔥 경유지 2 (있을 때만 표시) ---
                      if (item.waypoint2Address != null &&
                          item.waypoint2Address!.isNotEmpty) ...[
                        const SizedBox(height: 8),
                        Row(
                          children: [
                            Container(
