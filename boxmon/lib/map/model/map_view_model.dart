...
class MapViewModel extends GetxController {
  ...
  Future<void> submitShipmentRequest() async {
    try {
      ...
      String combinedDescription =
          "${widthController.text}x${lengthController.text}x${heightController.text} cm";
  ...
  Future<void> handleMapTap(NLatLng latLng, {String? buildingName}) async {
    try {
      ...
      if (finalAddr.isNotEmpty) {
        ...
        print(
          "📍 [Coordinates] 위도: ${latLng.latitude}, 경도: ${latLng.longitude}",
        );
      }
    } catch (e) {
  ...
  Future<NaverAddressModel?> _fetchAddress(double lat, double lng) async {
    ...
    print(
      "🔑 [Key Check] ID: ${clientId.isEmpty ? 'Empty' : 'OK'}, Secret: ${clientSecret.isEmpty ? 'Empty' : 'OK'}",
    );
